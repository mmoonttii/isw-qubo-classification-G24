...
def _resolve_input_path(user_path: str) -> Path:
    ...
    Per §12, the input to this module (``--in-normalized``) is the file
    that ``preprocessing.py`` just wrote into ``outputs/``. To make the
    exact CLI invocations in the spec work regardless of the caller's
    current working directory, this tries, in order:

    1. *user_path* exactly as given (absolute, or relative to cwd) —
       so a caller who does pass a full/relative path is honoured.
    2. ``outputs/<basename of user_path>`` — matching where a prior
       pipeline stage (or this module's own ``_to_output_path``) would
       have written it.
