...
def _resolve_data_dir() -> Path:
    ...
    Mirrors :func:`_resolve_outputs_dir`: walks upward from this file's
    location looking for a directory that already contains a ``data/``
    sub-directory (the repo root, per §4 of the spec), falling back to
...
def _resolve_input_path(user_path: str) -> Path:
    """Resolve *user_path* to an existing input CSV.

    Tries, in order:

    1. *user_path* exactly as given (absolute, or relative to cwd) —
       so a caller who passes a full/relative path is still honoured.
    2. ``data/<basename of user_path>`` — the ``data/`` directory
       mandated by §4 of the spec, where ``input_dataset.csv`` and any
       other dataset given to the evaluator is expected to live.
    ...
    fallback = _resolve_data_dir() / literal.name
    if fallback.exists():
        return fallback
