...
All tests run against a single, static, version-controlled dataset:

    data/sample_test_dataset.csv

This file is NOT generated at test time. It is a fixed artifact created
once via ``scripts/generate_sample_dataset.py`` and committed to the
repository, as required by spec §13. If the file is missing, the suite
fails fast with instructions on how to (re)create it, rather than
silently generating a substitute in-memory dataset.
...
# ─────────────────────────────────────────────────────────────────────────────
# NOTE: the N_SAMPLES / N_FEATURES / N_SPARSE_COLS / NAN_FRACTION / POS_FRACTION
# values below are not generation parameters anymore — they describe the KNOWN,
# FIXED structure of the static file data/sample_test_dataset.csv, exactly as
# produced by scripts/generate_sample_dataset.py. If that file is ever
# regenerated with different shape parameters, update these constants (and
# regenerate) together so they stay in sync.
...
SEED          = 42           # seed used when the static file was generated
...
_STATIC_DATASET_PATH = _PROJECT_ROOT / "data" / "sample_test_dataset.csv"


@pytest.fixture(scope="session")
def raw_csv() -> Path:
    """
    Returns the path to the **static, committed** sample dataset required
    by spec §13: ``data/sample_test_dataset.csv``.

    This fixture does NOT generate any data. The file is a fixed artifact
    built once via ``scripts/generate_sample_dataset.py`` (same shape as
    documented by the N_SAMPLES / N_FEATURES / N_SPARSE_COLS / NAN_FRACTION
    / POS_FRACTION constants above) and checked into version control, so
    that every test run — locally or on the evaluator's machine — exercises
    the exact same data.
    ...
    -------
    pathlib.Path
        Absolute path to ``data/sample_test_dataset.csv``.
    """
    if not _STATIC_DATASET_PATH.exists():
        pytest.fail(
            f"Static test dataset not found at '{_STATIC_DATASET_PATH}'. "
            "Per spec §13 this file must be a committed artifact, not "
            "generated at test time. Create it once with:\n\n"
            "    python scripts/generate_sample_dataset.py\n\n"
            "then commit data/sample_test_dataset.csv to the repository."
        )

    df = pd.read_csv(_STATIC_DATASET_PATH)

    if TARGET not in df.columns:
        pytest.fail(
            f"'{_STATIC_DATASET_PATH}' has no '{TARGET}' column. "
            "Regenerate it with scripts/generate_sample_dataset.py."
        )

    # Guard the §13 invariant (≥10% representation for each class) against
    # the file having been hand-edited or replaced with an unsuitable one.
    pos_frac = df[TARGET].mean()
    assert pos_frac >= 0.10, (
        f"Static dataset '{_STATIC_DATASET_PATH.name}' has only "
        f"{pos_frac:.1%} positive samples; §13 requires ≥ 10 %."
    )
    assert (1 - pos_frac) >= 0.10, (
        f"Static dataset '{_STATIC_DATASET_PATH.name}' has only "
        f"{1 - pos_frac:.1%} negative samples; §13 requires ≥ 10 %."
    )

    return _STATIC_DATASET_PATH
...
class TestPreprocessing:
    ...
    def _setup(self, preproc):
        """Store output paths and load the normalised CSV.

        The load is guarded by an explicit existence check so that, if
        fit_normalize() failed to write the file, tests report FAILED with
        a clear prerequisite message rather than ERROR from a raw
        FileNotFoundError raised inside the fixture.
        """
        self.norm_csv  = preproc["csv"]
        self.json_path = preproc["json"]
        if not Path(self.norm_csv).exists():
            pytest.fail(
                f"Prerequisite file not found: '{self.norm_csv}'.  "
                "fit_normalize() did not write the normalised CSV to the "
                "expected location.  Check the upstream preproc fixture."
            )
        self.df       = pd.read_csv(self.norm_csv)
...
class TestNormalization:
    ...
    def _setup(self, preproc, raw_csv):
        """Load the normalised CSV and record row counts for all tests.

        Every test in TestNormalization needs the feature DataFrame.  The
        guard converts a missing-file situation from a fixture ERROR into a
        FAILED test with a diagnostic message, rather than cascading into a
        raw FileNotFoundError that obscures the actual cause.
        """
        norm_csv = preproc["csv"]
        if not Path(norm_csv).exists():
            pytest.fail(
                f"Prerequisite file not found: '{norm_csv}'.  "
                "fit_normalize() did not produce the normalised CSV; "
                "normalisation tests cannot run."
            )
...
class TestFeatureSelection:
    ...
    def _setup(self, feat_sel, preproc):
        """Store output paths and load the feature-selection JSON.

        The guard on the JSON file converts a missing-file situation from a
        fixture ERROR (raw FileNotFoundError) into a FAILED test with a
        message that points to test_all_output_files_are_created as the
        authoritative failure report.
        """
        self.paths       = feat_sel
        self.preproc_csv = preproc["csv"]
        json_path        = feat_sel["json"]
        if not Path(json_path).exists():
            pytest.fail(
                f"Prerequisite file not found: '{json_path}'.  "
                "select_features() did not write the JSON summary; "
                "test_all_output_files_are_created should report the root cause."
            )
...
@pytest.fixture(scope="session")
def model_outputs(feat_sel) -> dict[str, str]:
    """
    ...
    Path contract
    -------------
    Unlike preprocessing.py and feature_selection.py, model.py's train()
    does NOT apply any internal path remapping.  It calls
    ``Path(model_path).parent.mkdir(parents=True, exist_ok=True)`` and then
    writes to exactly the path it received, so we may pass any full path.

    We choose ``_project_outputs_dir()`` for two reasons:
    • Consistency: all five pipeline fixtures now write to the same directory.
    • Predictability: the model file survives between ``pytest`` invocations,
      which is important because joblib files can be inspected for debugging
      and are expected in ``outputs/`` by the spec's CLI examples.
    ...
    out_dir = _project_outputs_dir()
    out = {
        "model_path":   str(out_dir / (_TEST_PREFIX + "model.joblib")),
        "metrics_json": str(out_dir / (_TEST_PREFIX + "training_metrics.json")),
    }
    train(
        classifier       = _MANDATORY_CLF,
        reducedTrain_csv = feat_sel["train_csv"],
        target_column    = TARGET,
        model_path       = out["model_path"],    # full path — model.py respects it
...
@pytest.fixture(scope="session")
def prediction_outputs(feat_sel, model_outputs) -> dict[str, str]:
    """
    ...
    Path contract
    -------------
    Same as ``model_outputs``: predict() in model.py respects any path, so we
    choose ``_project_outputs_dir()`` for consistency with the rest of the
    pipeline fixtures.  Full paths are passed directly — no bare-filename trick
    needed.
    ...
    out_dir = _project_outputs_dir()
    out = {
        "predictions_csv": str(out_dir / (_TEST_PREFIX + "predictions.csv")),
        "stats_json":      str(out_dir / (_TEST_PREFIX + "classification_stats.json")),
    }
    predict(
        reduced_Test_csv   = feat_sel["test_csv"],
        target_column      = TARGET,
        model_path         = model_outputs["model_path"],
        predictions_csv    = out["predictions_csv"],   # full path — predict() respects it
...
class TestModelTraining:
    ...
    def _setup(self, model_outputs):
        """Store output paths and load the training-metrics JSON.

        The guard converts a missing JSON file from a fixture ERROR into a
        FAILED test with a clear message, rather than a raw FileNotFoundError
        that makes the test report look like an infrastructure failure rather
        than an implementation failure.
        """
        self.model_path   = model_outputs["model_path"]
        self.metrics_json = model_outputs["metrics_json"]
        if not Path(self.metrics_json).exists():
            pytest.fail(
                f"Prerequisite file not found: '{self.metrics_json}'.  "
                "train() did not write the metrics JSON; "
                "test_training_metrics_json_is_created should report the root cause."
            )
        with open(self.metrics_json) as fh:
...
class TestModelPrediction:
    ...
    def _setup(self, prediction_outputs, feat_sel):
        """Store output paths and load all prediction artefacts.

        Two guards are used:
        • predictions_csv — the primary predict() output; if absent, predict()
          did not run or crashed before writing.
        • stats_json      — a secondary output; checked separately so a partial
          failure (CSV written but JSON not) is diagnosed independently.

        Both convert raw FileNotFoundErrors in the fixture into FAILED tests
        with messages that point to the relevant existence-check test.
        """
        self.paths    = prediction_outputs
        self.test_csv = feat_sel["test_csv"]

        predictions_csv = prediction_outputs["predictions_csv"]
        if not Path(predictions_csv).exists():
            pytest.fail(
                f"Prerequisite file not found: '{predictions_csv}'.  "
                "predict() did not write the predictions CSV; "
                "test_predictions_csv_is_created should report the root cause."
            )

        stats_json = prediction_outputs["stats_json"]
        if not Path(stats_json).exists():
            pytest.fail(
                f"Prerequisite file not found: '{stats_json}'.  "
                "predict() did not write the classification stats JSON; "
                "test_classification_stats_json_is_created should report the root cause."
            )
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
#  C L I   I N T E R F A C E   T E S T S   (spec §12)
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
class TestCLIInterfaces:
    """
    Validates that the command-line interfaces for all three modules parse
    arguments correctly and execute without crashing, as required by spec §12.
    """

    @pytest.fixture(autouse=True)
    def _setup(self):
        """Locate the Python executable and the script paths."""
        self.python_exe = sys.executable
        self.src_dir = _PROJECT_ROOT / "src" / "qubo_project"
        self.out_dir = _project_outputs_dir()

    def test_cli_preprocessing(self, raw_csv):
        """Tests: python preprocessing.py --input ..."""
        script = self.src_dir / "preprocessing.py"
        out_csv = "cli_test_normalized.csv"
        out_json = "cli_test_preprocessing.json"
        ...
        cmd = [
            ...
            "--out-data", out_csv,
            "--out-json", out_json,
            "--min-perc-valid", str(0.05)
        ]

        result = subprocess.run(cmd, capture_output=True, text=True)
        assert result.returncode == 0, (
            f"CLI preprocessing failed with exit code {result.returncode}.\n"
            f"STDERR: {result.stderr}"
        )
        assert (self.out_dir / out_csv).exists(), "CLI did not create output CSV."
        assert (self.out_dir / out_json).exists(), "CLI did not create output JSON."

    def test_cli_feature_selection(self, preproc):
        """Tests: python feature_selection.py --in-normalized ..."""
        script = self.src_dir / "feature_selection.py"
        out_train = "cli_test_train.csv"
        out_test = "cli_test_test.csv"
        out_opt = "cli_test_optimizations.csv"
        out_json = "cli_test_fs.json"

        cmd = [
            self.python_exe, str(script),
            "--in-normalized", preproc["csv"],
            "--out-train", out_train,
            "--out-test", out_test,
            "--out-optimizations", out_opt,
            "--out-json", out_json,
            "--target", TARGET,
            "--perc-selected", "0.20",
            "--allowance", "1",
            "--perc-test", "0.30",
            "--seed", str(SEED),
            "--alpha-computations", "2"  # Keep low for fast tests
        ]

        result = subprocess.run(cmd, capture_output=True, text=True)
        assert result.returncode == 0, (
            f"CLI feature_selection failed with exit code {result.returncode}.\n"
            f"STDERR: {result.stderr}"
        )
        assert (self.out_dir / out_train).exists(), "CLI did not create train CSV."
        assert (self.out_dir / out_json).exists(), "CLI did not create FS JSON."

    def test_cli_model_train(self, feat_sel):
        """Tests: python model.py train --classifier ..."""
        script = self.src_dir / "model.py"
        out_model = "cli_test_model.joblib"
        out_metrics = "cli_test_metrics.json"

        cmd = [
            self.python_exe, str(script), "train",
            "--classifier", _MANDATORY_CLF,
            "--in-reduced", feat_sel["train_csv"],
            "--target", TARGET,
            "--out-model", out_model,
        ...
        result = subprocess.run(cmd, capture_output=True, text=True)
        assert result.returncode == 0, (
            f"CLI model train failed with exit code {result.returncode}.\n"
            f"STDERR: {result.stderr}"
        )
        assert (self.out_dir / out_model).exists(), "CLI did not create model joblib."

    def test_cli_model_predict(self, feat_sel, model_outputs):
        """Tests: python model.py predict --input-testset ..."""
        script = self.src_dir / "model.py"
        out_preds = "cli_test_predictions.csv"
        out_stats = "cli_test_stats.json"

        cmd = [
            self.python_exe, str(script), "predict",
            "--input-testset", feat_sel["test_csv"],
            "--target", TARGET,
            "--model", model_outputs["model_path"],
            "--out-predictions", out_preds,
            "--out-stats", out_stats
        ]

        result = subprocess.run(cmd, capture_output=True, text=True)
        assert result.returncode == 0, (
            f"CLI model predict failed with exit code {result.returncode}.\n"
            f"STDERR: {result.stderr}"
        )
        assert (self.out_dir / out_preds).exists(), "CLI did not create predictions CSV."
        assert (self.out_dir / out_stats).exists(), "CLI did not create stats JSON."